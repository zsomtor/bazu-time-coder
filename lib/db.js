...
async function ensureTables() {
  ...
  `;
  // Migration for databases created before per-project checklist items existed.
  // Rows with a NULL project_id are the shared base template; rows with a
  // project_id are extra items belonging to a single project.
  await sql`
    ALTER TABLE checklist_template
    ADD COLUMN IF NOT EXISTS project_id INTEGER REFERENCES projects(id) ON DELETE CASCADE
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_checklist_template_project_id ON checklist_template(project_id)
  `;
  await sql`
